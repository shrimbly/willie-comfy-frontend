...
vi.mock('./MoshpitAddFilterPopover.vue', () => ({
  default: {
    ...
    template: '<button data-testid="moshpit-add-filter-trigger">Add filter</button>'
  }
}))
...
function makeChip(id: string, param: FilterChip['param'], value: FilterChip['value']): FilterChip {
...
describe('MoshpitFilterChipRow', () => {
  ...
  it('renders one chip per entry in filterStore.chips', () => {
    const filterStore = useMoshpitFilterStore()
    filterStore.addChip(makeChip('a', 'cfg', { kind: 'numeric', min: 6, max: 8, exact: null }))
    filterStore.addChip(makeChip('b', 'sampler', { kind: 'categorical', values: ['euler'] }))
    filterStore.addChip(makeChip('c', 'positivePrompt', { kind: 'text', substring: 'cat' }))
  ...
  it('clicking the × button calls filterStore.removeChip with the chip id', async () => {
    ...
    const filterStore = useMoshpitFilterStore()
    filterStore.addChip(makeChip('chip1', 'cfg', { kind: 'numeric', min: 7, max: 8, exact: null }))
  ...
  it('remove button has aria-label including the chip parameter display name', () => {
    const filterStore = useMoshpitFilterStore()
    filterStore.addChip(makeChip('s1', 'sampler', { kind: 'categorical', values: ['euler'] }))
  ...
  it('renders aria-live status with chip count', () => {
    const filterStore = useMoshpitFilterStore()
    filterStore.addChip(makeChip('x', 'cfg', { kind: 'numeric', min: null, max: null, exact: 7 }))
  ...
  it('renders correct value summary for numeric range chip', () => {
    const filterStore = useMoshpitFilterStore()
    filterStore.addChip(makeChip('n', 'cfg', { kind: 'numeric', min: 6, max: 8, exact: null }))
    mountChipRow()
  ...
  it('renders correct value summary for numeric exact chip', () => {
    const filterStore = useMoshpitFilterStore()
    filterStore.addChip(
      makeChip('e', 'steps', { kind: 'numeric', min: null, max: null, exact: 20 })
    )
    mountChipRow()
    const chip = screen.getByTestId('moshpit-filter-chip')
  ...
  it('renders correct value summary for categorical single value', () => {
    const filterStore = useMoshpitFilterStore()
    filterStore.addChip(makeChip('c1', 'sampler', { kind: 'categorical', values: ['euler'] }))
    mountChipRow()
    expect(screen.getByTestId('moshpit-filter-chip').textContent).toContain('euler')
  })

  it('renders correct value summary for categorical multi-value', () => {
    const filterStore = useMoshpitFilterStore()
    filterStore.addChip(
      makeChip('c2', 'sampler', { kind: 'categorical', values: ['euler', 'dpmpp_2m'] })
    )
    mountChipRow()
    expect(screen.getByTestId('moshpit-filter-chip').textContent).toContain('+1')
  })

  it('renders correct value summary for text chip (quoted)', () => {
    const filterStore = useMoshpitFilterStore()
    filterStore.addChip(
      makeChip('t1', 'positivePrompt', { kind: 'text', substring: 'masterpiece' })
    )
    mountChipRow()
    expect(screen.getByTestId('moshpit-filter-chip').textContent).toContain('"masterpiece"')
  })
  ...
  it('renders correct value summary for resolution single pair', () => {
    ...
    mountChipRow()
    expect(screen.getByTestId('moshpit-filter-chip').textContent).toContain('512')
  })
  ...
  it('renders correct value summary for boolean chip (favourite=true)', () => {
    const filterStore = useMoshpitFilterStore()
    filterStore.addChip(makeChip('b1', 'favourite', { kind: 'boolean', value: true }))
    mountChipRow()
    expect(screen.getByTestId('moshpit-filter-chip').textContent).toContain('Favourite')
  })
})
