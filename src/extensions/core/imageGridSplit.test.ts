...
function makeNode() {
  ...
  return {
    constructor: { comfyClass: 'ImageGridSplit' },
...
const extension = () =>
  capturedExtensions.find((e) => e.name === 'Comfy.ImageGridSplit')!

describe('Comfy.ImageGridSplit extension', () => {
  beforeAll(async () => {
    await import('./imageGridSplit')
  })
  ...
  it('injects a non-serialized preview widget at the top of the node', () => {
    ...
    expect(preview.type).toBe('imagegridsplit')
