...
const GRID_SPLIT_MAX = 16
const IMAGE_INPUT_SLOT = 0

function clampGridCount(value: unknown): number {
  const numeric = typeof value === 'number' ? value : Number(value)
  if (!Number.isFinite(numeric)) return GRID_SPLIT_MIN
  return Math.min(GRID_SPLIT_MAX, Math.max(GRID_SPLIT_MIN, Math.floor(numeric)))
}
...
export function useImageGridSplit(nodeId: string) {
