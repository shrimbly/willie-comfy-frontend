...
import { useImageGridSplit } from './useImageGridSplit'
...
type Result = ReturnType<typeof useImageGridSplit>

function mount(nodeId = 'node-1') {
  let result!: Result
  const Wrapper = defineComponent({
    setup() {
      result = useImageGridSplit(nodeId)
      return {}
    },
    render: () => null
  })
