...
test.describe('Moshpit asset pipeline', { tag: '@moshpit' }, () => {
  test(
    'cold cache shows processing pill and renders sprites progressively (ASSET-07)',
    async ({ comfyPage }) => {
  ...
  test(
    'warm cache skips the pill (ASSET-10 / D-08)',
    async ({ comfyPage }) => {
      const moshpit = new MoshpitCanvasHelper(comfyPage.page)
      await moshpit.goto()
  ...
  test(
    'cancel button aborts processing and the pill disappears (ASSET-08)',
    async ({ comfyPage }) => {
      const moshpit = new MoshpitCanvasHelper(comfyPage.page)
      await moshpit.goto()
