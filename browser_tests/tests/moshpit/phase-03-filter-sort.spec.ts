...
test.describe('@moshpit filter & sort', () => {
  test('workflow picker gates the canvas with an empty-state overlay (FILTER-01)', async ({
    comfyPage
  }) => {
    ...
    await expect(comfyPage.page.getByTestId('moshpit-filter-chip')).toHaveCount(
      0
    )
  })

  test('no axis labels render before a sort axis is set (SORT-01 baseline)', async ({
    comfyPage
  }) => {
    const moshpit = new MoshpitCanvasHelper(comfyPage.page)
    await moshpit.goto()
  ...
  test('settings panel contains workflow picker and time range picker (FILTER-01 / FILTER-06)', async ({
    comfyPage
  }) => {
    const moshpit = new MoshpitCanvasHelper(comfyPage.page)
    await moshpit.goto()
  ...
  test('show hidden toggle is present and accessible in settings panel (FILTER-11)', async ({
    comfyPage
  }) => {
    const moshpit = new MoshpitCanvasHelper(comfyPage.page)
    await moshpit.goto()
